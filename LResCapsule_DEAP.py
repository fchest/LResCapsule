...
class PrimaryCapsule(nn.Module):
    ...
    def forward(self, x):
        outputs = self.conv2d(x) #对输入进行卷积处理，这一步output的形状是[batch,out_channels,p_w,p_h] #torch.Size([100, 512, 8, 2])
        outputs = outputs.view(x.size(0), -1, self.dim_caps) #将4D的卷积输出变为3D的胶囊输出形式，output的形状为[batch,caps_num,dim_caps]，其中caps_num为胶囊数量 #torch.Size([100, 1024, 8])
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes):
        ...
        self.digitcaps = DenseCapsule(in_num_caps=64*8*2, in_dim_caps=8,
...
def deap_load(sub,dimention):
    # if debaseline == 'yes':
    #     dataset_suffix = "f_dataset.pkl"
    #     label_suffix = "_labels.pkl"
    #     dataset_dir = "D:/wangbin/dreamer/data_pre(-base)/" + dimention + "/"
    # else:
    #     dataset_suffix = "_rnn_dataset.pkl"
    #     label_suffix = "_labels.pkl"
    #     dataset_dir = '/media/data/wangjinqin/EEG/MTCA/deap_shuffled_data/' + 'no_' + dimention + '/'
    
    dataset_suffix = ".mat_win_128_rnn_dataset.pkl"
    label_suffix = ".mat_win_128_labels.pkl"

    dataset_dir = '/media/data/wangjinqin/EEG/DEAP/TIME/deap_shuffled_data/' + "yes" + "_" + "valence"  + "/"

    ###load training set
    with open(dataset_dir + sub + dataset_suffix, "rb") as fp:
        datasets = pickle.load(fp)
    with open(dataset_dir + sub + label_suffix, "rb") as fp:
    ...
    datasets = np.transpose(datasets.reshape(-1, 128, 32, 1), (0, 3, 1, 2))
...
dimentions = ['valence']#,'arousal','dominance']
# subjects = ['s04']
...
if __name__ == "__main__":
    ...
    for dimention in dimentions:
        for subject in subjects:
            ...
            dataset_name == 'deap'  # load dreamer data
            datasets, labels = deap_load(subject,dimention)
